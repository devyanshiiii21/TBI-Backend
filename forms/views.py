...
class TideEirAPIView(APIView):
    def post(self, request):
        serializer = TideEirSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        else:
...
class TideGrantAPIView(APIView):
    def post(self, request):
        serializer = TideGrantSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        else:
